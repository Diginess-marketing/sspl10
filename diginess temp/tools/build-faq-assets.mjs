// FAQ/Contact section art (Higgsfield gpt_image_2_5, sources in tools/assets-src) -> public/assets/faq-section
// The title is on flat white: key white to transparent (alpha from distance to white), trim, then avif/webp variants.
import sharp from 'sharp';
const OUT = 'public/assets/faq-section';
const SRC = 'tools/assets-src';

const { data, info } = await sharp(`${SRC}/faq-title.png`).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
...
for (const w of [tm.width]) {
  ...
  await img().webp({ quality: 85, alphaQuality: 92 }).toFile(`${OUT}/faq-title-${w === tm.width ? 'full' : w + 'w'}.webp`);
  await img().avif({ quality: 65 }).toFile(`${OUT}/faq-title-${w === tm.width ? 'full' : w + 'w'}.avif`);
}

for (const w of [640, 1168]) {
  const img = () => sharp(`${SRC}/faq-contact-bg.png`).resize({ width: w });
  await img().webp({ quality: 80 }).toFile(`${OUT}/faq-contact-bg-${w}w.webp`);
  await img().avif({ quality: 55 }).toFile(`${OUT}/faq-contact-bg-${w}w.avif`);
}
