// Partners section title lettering (Higgsfield gpt_image_2_5, source in tools/assets-src) -> public/assets/partners-section
// The title is on flat white: key white to transparent (alpha from distance to white), trim, then avif/webp variants.
import sharp from 'sharp';
const OUT = 'public/assets/partners-section';
const SRC = 'tools/assets-src';

const { data, info } = await sharp(`${SRC}/pt-title-a.png`).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
...
for (const w of [tm.width]) {
  ...
  await img().webp({ quality: 85, alphaQuality: 92 }).toFile(`${OUT}/pt-title-${w === tm.width ? 'full' : w + 'w'}.webp`);
  await img().avif({ quality: 65 }).toFile(`${OUT}/pt-title-${w === tm.width ? 'full' : w + 'w'}.avif`);
}
