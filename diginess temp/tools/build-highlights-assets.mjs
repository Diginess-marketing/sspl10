// Highlights section art (Higgsfield gpt_image_2_5, sources in tools/assets-src) -> public/assets/highlights-section
// The title is on flat white: key white to transparent (alpha from distance to white), trim, then avif/webp variants.
import sharp from 'sharp';
const OUT = 'public/assets/highlights-section';
const SRC = 'tools/assets-src';

const { data, info } = await sharp(`${SRC}/hl-title-a.png`).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
...
for (const w of [tm.width]) {
  ...
  await img().webp({ quality: 85, alphaQuality: 92 }).toFile(`${OUT}/hl-title-${w === tm.width ? 'full' : w + 'w'}.webp`);
  await img().avif({ quality: 65 }).toFile(`${OUT}/hl-title-${w === tm.width ? 'full' : w + 'w'}.avif`);
}
for (const w of [768, 1344]) {
  const img = () => sharp(`${SRC}/hl-bg.png`).resize({ width: w });
  await img().webp({ quality: 80 }).toFile(`${OUT}/hl-bg-${w}w.webp`);
  await img().avif({ quality: 55 }).toFile(`${OUT}/hl-bg-${w}w.avif`);
}
