// Spirit-of-street-cricket block art (Higgsfield gpt_image_2_5, sources in tools/assets-src) -> public/assets/spirit
// The title is on flat white: key white to transparent (alpha from distance to white), trim, then avif/webp variants.
import sharp from 'sharp';
const OUT = 'public/assets/spirit';
const SRC = 'tools/assets-src';

const { data, info } = await sharp(`${SRC}/spirit-title.png`).ensureAlpha().raw().toBuffer({ resolveWithObject: true });
...
for (const w of [tm.width]) {
  ...
  await img().webp({ quality: 85, alphaQuality: 92 }).toFile(`${OUT}/spirit-title-${w === tm.width ? 'full' : w + 'w'}.webp`);
  await img().avif({ quality: 65 }).toFile(`${OUT}/spirit-title-${w === tm.width ? 'full' : w + 'w'}.avif`);
}

for (const w of [768, 1344]) {
  const img = () => sharp(`${SRC}/spirit-bg.png`).resize({ width: w });
  await img().webp({ quality: 80 }).toFile(`${OUT}/spirit-bg-${w}w.webp`);
  await img().avif({ quality: 55 }).toFile(`${OUT}/spirit-bg-${w}w.avif`);
}
