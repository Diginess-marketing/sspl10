...
// Raw markdown table data provided by the user (Same as before)
const rawTableData = `
...
| SELECTED  |                        |            |          |   |
| 95        | M.MOHAMMED ASIF        | 6380973425 | SELECTED |   |
...
function parseTableData(raw) {
    ...
    return players.map(p => {
        ...
        // FIX: Ensure status is fully normalized
        // Sometimes "NOT SELECTED" becomes "NOT SELECTED" (correct)
        // But if split was bad, it might be weird.
        // Also check if status is "NOT" -> make it "NOT SELECTED"
        if (status === 'NOT') status = 'NOT SELECTED';
...
try {
    ...
    // 1. CLEANUP: Remove ANY records that have city "Trichy & Coimbatore"
    // This is to wipe the previous bad import
    const initialLength = players.length;
    players = players.filter(p => p.city !== "Trichy & Coimbatore");
    const cleanedLength = players.length;
    console.log(`Cleaned ${initialLength - cleanedLength} records from previous import.`);

    // 2. CHECK FOR DUPLICATES & MERGE
    // Create a map of existing players by mobile
    const existingPlayersMap = new Map();
    players.forEach((p, idx) => {
        if (p.mobile) existingPlayersMap.set(p.mobile, idx);
    });
    ...
    newPlayers.forEach(newP => {
        if (existingPlayersMap.has(newP.mobile)) {
            // Update existing record
            const idx = existingPlayersMap.get(newP.mobile);
            players[idx] = {
                ...players[idx], // Keep existing fields like proficiency if they exist? No, user implied these are new updates.
                // But actually, for "Duplicates", usually we want to keep the "master" ID but update status.
                // The prompt was "merge... and add these records".
                // So let's overwrite status and location.
                status: newP.status,
                city: newP.city,
                state: newP.state, // Update state too? Yes.
                // Don't overwrite name if it exists? 
                // Using the name from the new list might be safer if it's a correction.
                name: newP.name
                // Proficiency is N/A in new list, don't overwrite if existing has it.
            };
            // If existing had proficiency, keep it. If not, use N/A.
            if (!players[idx].proficiency) players[idx].proficiency = "N/A";
    ...
    fs.writeFileSync(playersDataPath, JSON.stringify(players, null, 2));
    console.log('Successfully re-imported data.');
