...
async function main() {
    const { data: c } = await supabase.from('trial_candidates').select('id, name, mobile').eq('mobile', '9884006284');
    for (const cand of c) {
        const { data: p } = await supabase.from('trial_progress').select('*').eq('candidate_id', cand.id);
        console.log(`Candidate ${cand.id}:`, p);
    }
}
