...
async function main() {
    const res = await supabase.from('trial_candidates')
        .select('id, trial_progress!inner(l1_called)', { count: 'exact', head: true })
        .eq('trial_progress.l1_called', false);

    console.log(`Count: ${res.count}, Data length: ${res.data?.length}`);
}
main();
