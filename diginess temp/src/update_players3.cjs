  ...
  try {
    ...
    for (const p of playersToUpdate) {
        ...
        if (regs && regs.length > 0) {
            console.log('Found in player_registrations:', regs.map(pr => ({id: pr.id, name: pr.full_name, phone: pr.phone, city: pr.city})));
            for (const r of regs) {
        ...
        if (cands && cands.length > 0) {
            console.log('Found in trial_candidates:', cands.map(pr => ({id: pr.id, name: pr.name, mobile: pr.mobile, state: pr.state})));
            for (const c of cands) {
                console.log(`Updating trial_candidates ID ${c.id}...`);
                const { error: updErr } = await supabase
                    .from('trial_candidates')
                    .update({ mobile: p['Mobile No'].toString(), state: p.Location })
                    .eq('id', c.id);
                if (updErr) console.log('Update Error:', updErr.message);
                else console.log('Update Success!');
            }
