...
export default function UTMAnalyticsDashboard() {
  const [activeTab, setActiveTab] = useState<'overview' | 'ga4' | 'registrations'>('overview');
  ...
  const [dateRange, setDateRange] = useState({
    startDate: '2024-12-01', // Extended range for testing
  ...
  const fetchRegistrations = useCallback(async () => {
    ...
    try {
      ...
      console.error('[UTMAnalytics] Registration fetch error details:', err);
  ...
  const loadAllData = useCallback(async () => {
    setLoading(true);
    setError(null);
    await Promise.all([fetchGA4Data(), fetchRegistrations()]);
    setLoading(false);
  }, [fetchGA4Data, fetchRegistrations]);
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="border-b border-gray-200">
            <nav className="flex -mb-px">
              {(['overview', 'ga4', 'registrations'] as const).map((tab) => (
                <button
                  key={tab}
                ...
                >
                  {tab === 'overview' ? 'Overview' : tab === 'ga4' ? 'GA4 Traffic' : 'Registrations'}
                </button>
              ))}
            </nav>
          </div>
