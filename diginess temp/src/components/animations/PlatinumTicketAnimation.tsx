...
interface PlatinumTicketAnimationProps {
    player: PlayerResult;
    onClose?: () => void;
    autoHideMs?: number;
    embedded?: boolean;
...
// Platinum Ticket celebration overlay for selected players (Level 3)
const PlatinumTicketAnimation: React.FC<PlatinumTicketAnimationProps> = ({ player, onClose, autoHideMs = 60000, embedded = false }) => {
    ...
    const handleDownload = async () => {
        ...
        const opt = {
            margin: 0,
            filename: `SSPL_PlayerPlatinum_Ticket_${player.name.replace(/\s+/g, '_')}.pdf`,
    ...
    const ticketContent = (
        <div className={`relative z-10 w-full max-w-xl mx-auto transition-all duration-700 ${isRotating ? 'animate-[spin_4s_linear_infinite]' : (embedded ? '' : 'animate-in zoom-in-95 duration-300 ease-out')}`} style={{ transformStyle: 'preserve-3d' }}>
            ...
            {/* Ticket Body */}
            <div ref={ticketRef} className="relative bg-linear-to-br from-[#E2E8F0] via-[#F8FAFC] to-[#CBD5E1] rounded-2xl p-1.5 shadow-2xl border-2 border-slate-300/80">
                <div className="bg-linear-to-b from-gray-50 via-slate-50 to-gray-100 rounded-xl border-4 border-double border-slate-500 p-5 relative overflow-hidden">
                    <div className="text-center space-y-3 relative z-10">
                        ...
                        <div className="relative py-1">
                            <h2 className="text-3xl sm:text-4xl font-heading font-extrabold tracking-tighter mb-2 leading-none" style={{ color: '#1E293B' }}>
                                PLAYER PLATINUM<br />TICKET
                            </h2>
                            <div className="h-1 w-36 bg-linear-to-r from-transparent via-slate-500 to-transparent mx-auto rounded-full"></div>
                        </div>
                        ...
                        <div className="pt-2">
                            <div className="inline-block bg-linear-to-r from-slate-500 via-gray-500 to-slate-600 text-white px-5 py-2 rounded-full shadow-lg border-2 border-slate-300/50">
                                <span className="font-black tracking-wide flex items-center gap-2 text-sm">
                                    <Star className="w-4 h-4 fill-slate-300" />
...
export default PlatinumTicketAnimation;
