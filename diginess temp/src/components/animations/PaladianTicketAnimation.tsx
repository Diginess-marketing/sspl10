...
interface PaladianTicketAnimationProps {
    player: PlayerResult;
    onClose?: () => void;
    autoHideMs?: number;
    embedded?: boolean;
...
// Paladian Ticket celebration overlay for selected players (Level 4)
const PaladianTicketAnimation: React.FC<PaladianTicketAnimationProps> = ({ player, onClose, autoHideMs = 60000, embedded = false }) => {
    ...
    const handleDownload = async () => {
        ...
        const opt = {
            margin: 0,
            filename: `SSPL_PlayerPaladian_Ticket_${player.name.replace(/\s+/g, '_')}.pdf`,
    ...
    const ticketContent = (
        <div className={`relative z-10 w-full max-w-xl mx-auto transition-all duration-700 ${isRotating ? 'animate-[spin_4s_linear_infinite]' : (embedded ? '' : 'animate-in zoom-in-95 duration-300 ease-out')}`} style={{ transformStyle: 'preserve-3d' }}>
            ...
            {/* Ticket Body */}
            <div ref={ticketRef} className="relative bg-linear-to-br from-[#E9D5FF] via-[#D8B4FE] to-[#C084FC] rounded-2xl p-1.5 shadow-2xl border-2 border-purple-300/80">
                <div className="bg-linear-to-b from-purple-50 via-purple-100 to-purple-200 rounded-xl border-4 border-double border-purple-500 p-5 relative overflow-hidden">
                    <div className="text-center space-y-3 relative z-10">
                        ...
                        <div className="relative py-1">
                            <h2 className="text-3xl sm:text-4xl font-heading font-extrabold tracking-tighter mb-2 leading-none" style={{ color: '#1E293B' }}>
                                PLAYER PALADIAN<br />TICKET
                            </h2>
                            <div className="h-1 w-36 bg-linear-to-r from-transparent via-purple-500 to-transparent mx-auto rounded-full"></div>
                        </div>
                        ...
                        <div className="pt-2">
                            <div className="inline-block bg-linear-to-r from-purple-500 via-purple-600 to-purple-700 text-white px-5 py-2 rounded-full shadow-lg border-2 border-purple-300/50">
                                <span className="font-black tracking-wide flex items-center gap-2 text-sm">
                                    <Star className="w-4 h-4 fill-purple-300" />
                                    QUALIFIED FOR LEVEL 5
                                    <Star className="w-4 h-4 fill-purple-300" />
                                </span>
                            </div>
                        </div>
...
export default PaladianTicketAnimation;
