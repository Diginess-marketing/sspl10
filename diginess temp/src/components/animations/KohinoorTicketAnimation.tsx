...
interface KohinoorTicketAnimationProps {
  player: PlayerResult;
  onClose?: () => void;
  autoHideMs?: number;
  embedded?: boolean;
...
// Kohinoor Ticket celebration overlay for selected players (Level 2)
const KohinoorTicketAnimation: React.FC<KohinoorTicketAnimationProps> = ({ player, onClose, autoHideMs = 60000, embedded = false }) => {
  ...
  const ticketContent = (
    <div className={`relative z-10 w-full max-w-xl mx-auto transition-all duration-700 ${isRotating ? 'animate-[spin_4s_linear_infinite]' : (embedded ? '' : 'animate-in zoom-in-95 duration-300 ease-out')}`} style={{ transformStyle: 'preserve-3d' }}>
      ...
      {/* Ticket Body */}
      <div ref={ticketRef} className="relative bg-linear-to-br from-[#E0F7FA] via-[#B2EBF2] to-[#80DEEA] rounded-2xl p-1.5 shadow-2xl border-2 border-cyan-300/80">
        <div className="bg-linear-to-b from-slate-50 via-cyan-50 to-blue-50 rounded-xl border-4 border-double border-blue-600 p-5 relative overflow-hidden">
          <div className="text-center space-y-3 relative z-10">
            ...
            <div className="relative py-1">
              <h2 className="text-4xl sm:text-5xl font-heading font-extrabold tracking-tighter mb-2 leading-none" style={{ color: '#1E3A8A' }}>
                KOHINOOR<br />TICKET
              </h2>
              <div className="h-1 w-36 bg-linear-to-r from-transparent via-blue-500 to-transparent mx-auto rounded-full"></div>
            </div>

            <div className="space-y-3">
              <div className="bg-white/60 border-2 border-blue-300/80 rounded-lg p-3 shadow-inner">
                <p className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: '#1E3A8A' }}>Player Name</p>
                <h3 className="text-2xl font-black tracking-tight" style={{ color: '#172554' }}>
                  {player.name}
                </h3>
              </div>
              <div className="bg-white/40 rounded-lg p-3 border-2 border-blue-300/60">
                <p className="text-xs font-bold uppercase mb-1 tracking-wider" style={{ color: '#1E3A8A' }}>Mobile Number</p>
                <p className="font-black text-xl" style={{ color: '#172554' }}>{player.mobile}</p>
              </div>
            </div>

            <div className="pt-2">
              <div className="inline-block bg-linear-to-r from-blue-500 via-cyan-500 to-blue-600 text-white px-5 py-2 rounded-full shadow-lg border-2 border-cyan-300/50">
...
export default KohinoorTicketAnimation;
