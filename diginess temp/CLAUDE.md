...
## Image generation rule (Higgsfield MCP — credit budget)
- **One image per generation call:** always `count: 1`. Never request variants/batches (count 2–4) unless the user explicitly asks.
- **Max 2 tries for the same image:** the first generation + at most ONE retry (only for a concrete defect, e.g. misspelled text). After the 2nd try, stop: use the better result, or fall back to existing assets/CSS, or ask the user. Never a 3rd try.
- Before generating: search `public/` for an existing asset first, and preflight cost with `get_cost: true`. Use `gpt_image_2_5` (~1 credit).
- Generate only what a section really needs (lettering, no-people backdrop). Never generate people/faces for real players, staff or advisors — use their supplied photos.
- Write the exact prompt once, carefully (exact spelling in quotes, palette hexes, "no other text"); don't iterate by trial and error.
