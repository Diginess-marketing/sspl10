        ...
        ga4History.forEach((row: any) => {
          ...
          for (let i = 0; i < count; i++) {
            expandedList.push({
              ...
              payment_status: 'N/A',
              date: row.date ? `${row.date.substring(0,4)}-${row.date.substring(4,6)}-${row.date.substring(6,8)}` : undefined
            });
          }
        });
    ...
    async function fetchMetrics() {
      try {
        ...
        const { data: regsData, error } = await supabase
          ...
          .select('full_name, phone, email, city, state, status, payment_status, school_name, created_at')
        ...
        if (regsData) {
          // Strict location filter for Coimbatore metrics, excluding test emails, and explicit inclusion
          const filteredRegs = regsData.filter((r: any) => {
            // Remove test records
            if (r.email && r.email.toLowerCase() === 'gokulnath.4c@gmail.com') {
              return false;
            }
            
            // Only include if city is Coimbatore, OR if it's the explicitly included player
            const cityMatch = r.city && r.city.toLowerCase().includes('coimbatore');
            const explicitInclude = r.phone === '8861468276';
            
            return cityMatch || explicitInclude;
          });
          
          if (isMounted) setRawRegistrations(filteredRegs);
          totalDBRegistrations = filteredRegs.length;
        ...
        if (isMounted) {
          ...
          setMetrics({
            ...
            successfulRegistrations: Math.max(ga4Conversions, successfulDBRegistrations),
