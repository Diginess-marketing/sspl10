    ...
    try {
        if (type === 'in') {
            ...
            const [
                ...
                supabase.from('trial_view').select('name, mobile, email, state, city, proficiency').eq('l1_attendance', 'ABSENT'),
                supabase.from('trial_view').select('name, mobile, email, state, city, proficiency').eq('l3_result', 'SELECTED'),
                supabase.from('trial_view').select('name, mobile, email, state, city, proficiency').eq('l2_result', 'SELECTED').or('l3_result.is.null,l3_result.eq.PENDING').or('l3_attendance.is.null,l3_attendance.neq.ABSENT'),
                supabase.from('trial_view').select('name, mobile, email, state, city, proficiency').eq('l2_result', 'SELECTED').eq('l3_attendance', 'ABSENT'),
                supabase.from('trial_view').select('name, mobile, email, state, city, proficiency').eq('l1_result', 'SELECTED').or('l2_result.is.null,l2_result.eq.PENDING'),
                supabase.from('trial_view').select('name, mobile, email, state, city, proficiency').is('l1_called', null),
            ...
            const notCalledNew = capturedData
                ?.filter((d: any) => !calledPhones.has(d.phone))
                ?.map((p: any) => ({ name: p.full_name, phone: p.phone, email: p.email, city: p.city || '-', state: p.state, proficiency: formatProficiency(p.position), status: 'Not Called' })) || [];
                
            playersList = [
                ...(completelyAbsent?.map((p: any) => ({ name: p.name, phone: p.mobile, email: p.email, city: p.city || '-', state: p.state, proficiency: formatProficiency(p.proficiency), status: 'Completely Absent' })) || []),
                ...(fullySelected?.map((p: any) => ({ name: p.name, phone: p.mobile, email: p.email, city: p.city || '-', state: p.state, proficiency: formatProficiency(p.proficiency), status: 'Fully Selected' })) || []),
                ...(l2Selected?.map((p: any) => ({ name: p.name, phone: p.mobile, email: p.email, city: p.city || '-', state: p.state, proficiency: formatProficiency(p.proficiency), status: 'Level 2 Selected' })) || []),
                ...(l3Absent?.map((p: any) => ({ name: p.name, phone: p.mobile, email: p.email, city: p.city || '-', state: p.state, proficiency: formatProficiency(p.proficiency), status: 'Level 3 Absent' })) || []),
                ...(l1Selected?.map((p: any) => ({ name: p.name, phone: p.mobile, email: p.email, city: p.city || '-', state: p.state, proficiency: formatProficiency(p.proficiency), status: 'Level 1 Selected' })) || []),
                ...(notCalledTrial?.map((p: any) => ({ name: p.name, phone: p.mobile, email: p.email, city: p.city || '-', state: p.state, proficiency: formatProficiency(p.proficiency), status: 'Not Called' })) || []),
                ...notCalledNew
            ];
            ...
            playersList = data?.map((p: any) => ({ name: p.name, phone: p.mobile, email: p.email, city: p.city || '-', state: p.state, proficiency: formatProficiency(p.proficiency), status: 'Out' })) || [];
                ...
                const { data } = await supabase.from('trial_view')
                    .select('name, mobile, email, state, city, proficiency')
                    .eq('l3_result', 'SELECTED');
                playersList = data?.map((p: any) => ({ name: p.name, phone: p.mobile, email: p.email, city: p.city || '-', state: p.state, proficiency: formatProficiency(p.proficiency), status: 'Finally Selected' })) || [];
            } else if (type === 'in_split_1') {
                const { data } = await supabase.from('trial_view')
                    .select('name, mobile, email, state, city, proficiency')
                    .eq('l1_result', 'SELECTED')
                    .or('l2_result.is.null,l2_result.eq.PENDING');
                playersList = data?.map((p: any) => ({ name: p.name, phone: p.mobile, email: p.email, city: p.city || '-', state: p.state, proficiency: formatProficiency(p.proficiency), status: 'Level 1 Selected' })) || [];
            } else if (type === 'in_split_2') {
                const { data } = await supabase.from('trial_view')
                    .select('name, mobile, email, state, city, proficiency')
                    .eq('l2_result', 'SELECTED')
                    .eq('l3_attendance', 'ABSENT');
                playersList = data?.map((p: any) => ({ name: p.name, phone: p.mobile, email: p.email, city: p.city || '-', state: p.state, proficiency: formatProficiency(p.proficiency), status: 'Level 2 selected but Level 3 Absent' })) || [];
                ...
                const { data: latestImportData } = await supabase.from('trial_candidates').select('imported_at').order('imported_at', { ascending: false }).limit(1);
                const lastImportDate = latestImportData?.[0]?.imported_at || '1970-01-01T00:00:00.000Z';
                const [{ data: uncalledTrialData }, { data: newRegistrationsData }] = await Promise.all([
                    supabase.from('trial_view')
                        .select('name, mobile, email, state, city, proficiency')
                        .or('l1_called.is.null,l1_called.eq.false'),
                    supabase.from('player_registrations')
                        .select('full_name, phone, email, state, city, position')
                        .in('payment_status', ['captured', 'completed', 'paid', 'success', 'CAPTURED', 'COMPLETED', 'PAID', 'SUCCESS'])
                        .gt('created_at', lastImportDate)
                ...
                playersList = [
                    ...(uncalledTrialData || []).map((p: any) => ({
                        name: p.name,
                        phone: p.mobile,
                        email: p.email,
                        city: p.city || '-',
                        state: p.state,
                        proficiency: formatProficiency(p.proficiency),
                        status: 'Not Called For'
                    })),
                    ...(newRegistrationsData || []).map((p: any) => ({
                        name: p.full_name,
                        phone: p.phone,
                        email: p.email,
                        city: p.city || '-',
                        state: p.state,
                        proficiency: formatProficiency(p.position),
                        status: 'Not Called For (New)'
                    }))
                ];
