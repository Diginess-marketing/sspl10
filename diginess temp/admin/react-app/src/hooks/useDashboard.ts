  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      const [
        ...
        supabase.from('trial_view').select('*', { count: 'exact', head: true }).eq('l1_attendance', 'ABSENT'), // count0: Completely Absent
        supabase.from('trial_view').select('*', { count: 'exact', head: true }).eq('l3_result', 'SELECTED'), // count1: Finally Selected
        supabase.from('trial_view').select('*', { count: 'exact', head: true }).eq('l2_result', 'SELECTED').or('l3_result.is.null,l3_result.eq.PENDING').or('l3_attendance.is.null,l3_attendance.neq.ABSENT'), // count2: L2 Selected
        supabase.from('trial_view').select('*', { count: 'exact', head: true }).eq('l2_result', 'SELECTED').eq('l3_attendance', 'ABSENT'), // count3: L3 Absent
        supabase.from('trial_view').select('*', { count: 'exact', head: true }).eq('l1_result', 'SELECTED').or('l2_result.is.null,l2_result.eq.PENDING'), // count4: L1 Selected
        supabase.from('trial_view').select('*', { count: 'exact', head: true }).or('proficiency.ilike.%BATSMAN%,proficiency.ilike.%BATTING%').eq('l3_result', 'SELECTED'),
        supabase.from('trial_view').select('*', { count: 'exact', head: true }).or('proficiency.ilike.%BOWLER%,proficiency.ilike.%BOWLING%').eq('l3_result', 'SELECTED'),
        supabase.from('trial_view').select('*', { count: 'exact', head: true }).or('proficiency.ilike.%ALL ROUNDER%,proficiency.ilike.%AR%,proficiency.ilike.%ALL-ROUNDER%,proficiency.is.null,proficiency.eq.,proficiency.eq.NA,proficiency.ilike.N/A%').eq('l3_result', 'SELECTED'),
      ...
      const inStationTotal = (count1 || 0) + (count3 || 0) + (count0 || 0) + (count4 || 0) + notCalledForCount;
      ...
      setStats({
        ...
        inPlayersSplit: [
          ...
          { label: 'Level 1 Selected', count: count4 || 0, icon: '🌟', description: 'Selected in Level 1 (Pending L2)' },
          { label: 'Level 2 selected but Level 3 Absent', count: count3 || 0, icon: '👀', description: 'Attended Level 2, Absent Level 3' },
